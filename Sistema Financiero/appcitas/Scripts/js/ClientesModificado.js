        ...
        posting.done(function (data, status, xhr) {
            ...
            if (dataCitas.length > 0) {
                for (var i = dataCitas.length - 1; i >= 0; i--) {
                    ...
                    //$('#CodigoCliente').val(dataCitas[i].ClienteId);
...
function addRow(ArrayData, tableID) {
    ...
    var newRow = $(tableID).dataTable().fnAddData([
        ...
        ArrayData['Cel'],
        ArrayData['Correo'],
        "<button data-id='" + ArrayData['ClienteId'] + "' data-name='" + ArrayData['Identidad'] + "' title='Ver Prestamos' data-toggle='tooltip' onClick='location.href=\"../Prestamos/Prestamo/" + ArrayData['ClienteId'] + "\"' id='btnVerRazones" + ArrayData['ClienteId'] + "' class='btn btn-success text-center btn-sm'><i class='fa fa-eye'></i></button> &nbsp" +
        ...
        "<button data-id='" + ArrayData['ClienteId'] + "' data-name='" + ArrayData['Nombre'] + "' title='Editar Información del Cliente' data-toggle='tooltip' onClick='verCliente(event)' id='btnEditarTipoRazon" + ArrayData['ClienteId'] + "' class='btn btn-primary text-center btn-sm'><i class='fa fa-pencil-square-o'></i></button>", // +
