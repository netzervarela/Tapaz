...
//GENERA PDF DEL RECIBO PAGO DE PRESTAMO

function GenerarPdfReciboPrestamo(Nombre, RE_Documento, RE_Numero, RE_Fecha, Tipo, PRES_Saldo, RE_Total_Rec) {
    var path = urljs + "/Caja/GeneraReciboPagoPrestamo";
    var posting = $.post(path, {
        Nombre: Nombre,
        RE_Documento: RE_Documento,
        RE_Numero: RE_Numero,
        RE_Fecha: RE_Fecha,
        Tipo: Tipo,
...
//OBTIENE INFO DEL RECIBO Y LLAMA LA FUNCION QUE GENERA EL PDF

function GenerarReciboPrestamo(Recibo) {

    try {
        var path = urljs + "/Caja/GeneraReciboPagoPrestamo1";
        var posting = $.post(path, { Recibo: Recibo });
        posting.done(function (data, status, xhr) {
            dataCitas = [];
            dataCitas = data;
            if (dataCitas.length > 0) {
                for (var i = dataCitas.length - 1; i >= 0; i--) {
                    ...
                    RE_Documento = dataCitas[i].RE_Documento
                    RE_Fecha = dataCitas[i].RE_Fecha
                    Tipo = dataCitas[i].Tipo
                    PRES_Saldo = dataCitas[i].PRES_Saldo
                    RE_Total_Rec = dataCitas[i].RE_Total_Rec
                    GenerarPdfReciboPrestamo(Nombre, RE_Documento, RE_Numero, RE_Fecha, Tipo, PRES_Saldo, RE_Total_Rec)
                }
            }
        });
...
var bandera = 0;
$("#btnAjustePrestamo").on('click', function (e) {
    if (bandera == 0) {
...
$("#btnGeneraPagoPrestamo").on('click', function (e) {
    ...
    if (bandera == 1) {
        ...
        GenerarAjustePrestamo($("#Prestamo").val(), $("#Capital").val(), $("#Intereses").val(), $("#Mora").val(), $("#TipoPago").val(), $("#Observacion").val(), $("#txt_fecha1").val());
...
function GenerarAjustePrestamo(codigo, Capital, Intereses, Mora, TipoPago, Observacion, FechaAjustePrestamo) {
    try {
        var path = urljs + "/Caja/PagoPrestamoAjuste";
        var posting = $.post(path, { codigo: codigo, Capital: Capital, Intereses: Intereses, Mora: Mora, TipoPago: TipoPago, Observacion: Observacion, FechaAjustePrestamo: FechaAjustePrestamo });
        posting.done(function (data, status, xhr) {
