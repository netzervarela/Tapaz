...
$('#btn_crear_reporte').on('click', function () {
    try {
        ...
        if (Validar(inputs, mensaje)) {
            var path = urljs + 'Reportes/ReportesClientes';
            ...
            posting.done(function (data, status, xhr) {
                ...
                LimpiarTablaExcel("#tableCitasDashboard1", excel_cols, 'ReportesClientes');
                if (data.length > 0) {
                    if (data[0].Accion > 0) {
                        var counter = 1;
...
function addRowDashboard1(ArrayData, tableID, counter) {
    var newRow = $(tableID).dataTable().fnAddData([
        ...
        ArrayData['CLI_Fecha_Nacimiento'],
        ArrayData['CLI_Direccion'],
        ArrayData['CLI_Agrego'],
        ArrayData['CLI_Fecha_Agrego']
    ]);
